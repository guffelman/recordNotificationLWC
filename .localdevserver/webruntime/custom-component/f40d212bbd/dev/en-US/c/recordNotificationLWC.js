    ...
    class RecordNotificationLWC extends lwc.LightningElement {
      ...
      renderedCallback() {
        switch (this.type) {
          case 'warning':
            this.symbol = '!';
            if (this.template.querySelector('[data-id="warning-card"]')) {
              this.template.querySelector('[data-id="warning-card"]').className = 'warning';
            }
            break;
          case 'error':
            this.symbol = 'X';
            if (this.template.querySelector('[data-id="warning-card"]')) {
              this.template.querySelector('[data-id="warning-card"]').className = 'error';
            }
            break;
          case 'success':
            this.symbol = '✓';
            if (this.template.querySelector('[data-id="warning-card"]')) {
              this.template.querySelector('[data-id="warning-card"]').className = 'success';
            }
            break;
          case 'info':
            this.symbol = 'i';
            if (this.template.querySelector('[data-id="warning-card"]')) {
              this.template.querySelector('[data-id="warning-card"]').className = 'info';
            }
            break;
          default:
            this.symbol = '!';
